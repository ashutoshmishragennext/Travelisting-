...
const MainButton = forwardRef<HTMLButtonElement, MainButtonProps>(
  (
    ...
    const propWidth = width === "full_width" ? "w-full" : width ? width : "w-[5rem]";
    const isSecondaryVariant = variant !== "primary";
    const size_height = size === "normal" 
      ? "h-[2.5rem]" 
      : size === "large" 
        ? "h-[3.75rem]" 
        : "h-[2.625rem]";
    const variant_hover = variant === "primary" 
      ? "hover:bg-blue-400" 
      : "hover:bg-blue-400";
    ...
    const commonProps = {
      form,
      ref,
      type: isSubmitable ? "submit" : "button" as "button" | "submit", // Explicitly assert the type
    ...
    return (
      <Button
        {...commonProps}
        onClick={!disabled && !isLoading ? action : undefined}
      >
        {isLoading ? (
          <>
